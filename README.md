...
Frontend: Next.js app with MediaRecorder API and Socket.io client (frontend).
Backend: Node.js + Express + Socket.io handling chunk ingestion, session orchestration, and API endpoints (backend).
DB: MongoDB stores session metadata and transcripts.
Storage: AWS S3 stores chunks and merged media.
Queue/Workers: AWS SQS queues drive FFmpeg merge and Deepgram transcription workers (worker).
...
Run Locally (quick) Prereqs: Node 16+, ffmpeg, MongoDB, AWS credentials with S3 + SQS access, Deepgram API key.
...
Backend socket handler: backend/src/wsHandler.js (open)
Session model: backend/src/models/Session.js (open)
Merge worker: worker/mergeWorker.js (open)
Transcribe worker: worker/transcribeWorker.js (open)
Frontend recorder/proctoring: frontend/components/Recorder.jsx (open)
Recruiter UI: frontend/pages/recruiter (open)
Demo & Walkthrough
